...
describe(`编辑工作流`, () => {
  it(`编辑现有板块时字段和字号直接同步到正文`, () => {
    ...
    expect(editorStore).toContain(`undoAction`)
  ...
  it(`390 关栏、文件夹标题、查找和导出按验收收口`, () => {
    ...
    expect(edit).toContain(`undoEdit`)
    expect(readSource(`apps/web/src/stores/editor.ts`)).toContain(`undoAction`)
  ...
  it(`编辑器宿主晚出现或被分栏换掉时，会重新挂到当前节点`, () => {
    const editor = readSource(`apps/web/src/views/CodemirrorEditor.vue`)
    expect(editor).toContain(`inner-\${showStyleRail}`)
