...
const panel = computed(() => {
  ...
  // 板块库不占中间栏；从左边出来，预览留在场上
  if (active === `blocks`) {
    return isMobile.value ? null : `blocks`
  }
  ...
  if (isMobile.value && active !== `folder`) {
    return null
  }
...
const showBackdrop = computed(() => {
  return panel.value === `posts` || panel.value === `folder`
})
...
<template>
  <Teleport to="body">
    <Transition name="workspace-drawer-fade">
      <div v-if="showBackdrop" class="fixed inset-0 z-55 bg-black/35 backdrop-blur-[2px]" @click="close" />
    </Transition>

    <Transition name="workspace-drawer-slide">
      <aside
        v-if="panel"
        class="fixed bottom-0 left-0 top-0 z-60 flex w-[min(94vw,420px)] flex-col border-r bg-background shadow-2xl"
      >
