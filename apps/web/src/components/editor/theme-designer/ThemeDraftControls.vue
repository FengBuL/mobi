...
import { ChevronDown, Download, GitCompareArrows, Palette, Redo2, RotateCcw, Save, Undo2, Upload } from 'lucide-vue-next'
...
function save() {
  ...
  if (designerStore.sourceTheme) {
    ...
    toast.success(`版式「${designerStore.sourceTheme.name}」已更新`)
    return
  }
...
async function handleImport(event: Event) {
  ...
  toast.success(`已导入版式「${created.name}」并应用`)
}
...
<template>
  <div class="space-y-3 border-t pt-3">
    <button
      type="button"
      class="flex min-h-9 w-full items-center justify-between gap-2 rounded-lg px-1 text-left text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      :aria-expanded="isExpanded"
      @click="isExpanded = !isExpanded"
    >
      版式微调
      <ChevronDown class="size-4 transition-transform" :class="{ '-rotate-90': !isExpanded }" />
    </button>
    ...
    <template v-if="isExpanded">
      ...
      <div class="flex flex-wrap gap-2">
        <Button variant="outline" size="sm" class="h-8 px-3 text-xs" :disabled="!designerStore.hasOverrides" @click="openSaveAs">
          另存为新版式
        </Button>
        <Button size="sm" class="h-8 px-3 text-xs" @click="save">
          <Save class="mr-1.5 size-3.5" />
          {{ designerStore.sourceTheme ? '保存修改' : '保存版式' }}
        </Button>
      </div>
      ...
      <Dialog v-model:open="isSaveAsOpen">
        <DialogContent class="sm:max-w-[420px]">
          <DialogHeader>
            <DialogTitle>保存为自定义版式</DialogTitle>
          </DialogHeader>
          <div class="space-y-2">
            <label class="text-sm font-medium">版式名称</label>
            <Input v-model="saveAsName" placeholder="输入版式名称" @keydown.enter="confirmSaveAs" />
          </div>
          <DialogFooter>
