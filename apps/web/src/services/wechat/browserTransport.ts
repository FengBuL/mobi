...
export function createBrowserWechatTransport(): WechatTransport {
  return {
    ...
    requestStableToken({ appID, appsecret, proxyOrigin }: StableTokenInput) {
      const origin = normalizeMpProxyOrigin(proxyOrigin) || WECHAT_API_ORIGIN

      return fetch<any, WechatStableTokenResult>(`${origin}/cgi-bin/stable_token`, {
        method: `POST`,
        data: {
    ...
    uploadImage({ accessToken, proxyOrigin, endpoint, file }: UploadImageInput) {
      const origin = normalizeMpProxyOrigin(proxyOrigin) || WECHAT_API_ORIGIN
      ...
      return fetch<any, WechatUploadImageResult>(`${origin}${UPLOAD_PATHNAMES[endpoint]}${query}`, {
        method: `POST`,
        data: formdata,
      })
    },
