...
function toBodyEm(raw, headingScale, tokens = new Map()) {
  ...
  if (em) return Number(em[1]) * headingScale
  const px = v.match(/^(-?[\d.]+)px$/)
  if (px) return Number(px[1]) / 16
  const pct = v.match(/^(-?[\d.]+)%$/)
  if (pct) return null // 百分比是相对宽度，跟纵向节奏无关
  return null
}
  ...
  for (const r of rules(css)) {
    for (const sel of r.selector.split(',').map(s => s.trim())) {
      ...
      if (fs_) {
        ...
        if (inner) rec.scale = Number(inner[1])
        else {
          const plain = fs_.match(/\*\s*([\d.]+)\s*\)/)
          if (plain) rec.scale = Number(plain[1])
        }
      }
      const mg = d.get('margin')
      if (mg) rec.margin = varFallback(mg) || mg
      if (d.get('margin-top')) rec.marginTop = varFallback(d.get('margin-top')) || d.get('margin-top')
      if (d.get('margin-bottom')) rec.marginBottom = varFallback(d.get('margin-bottom')) || d.get('margin-bottom')
    }
  }
  return out
})()
...
function analyze(name) {
  ...
  for (const r of rs) {
    if (!/(^|,)\s*(section|container)\s*(,|$)/.test(r.selector)) continue
    for (const [k, v] of decls(r.body)) {
      if (k.startsWith('--sk-')) tokens.set(k, v)
    }
  }
  ...
  for (const tag of ['h1', 'h2', 'h3', 'h4']) {
    ...
    if (tokScale) scale = Number(tokScale)
    ...
    if (SKELETON_DEFAULT[tag]?.marginTop) marginTop = SKELETON_DEFAULT[tag].marginTop
    if (SKELETON_DEFAULT[tag]?.marginBottom) marginBottom = SKELETON_DEFAULT[tag].marginBottom

    // 主题里的直接元素规则覆盖（后写的赢）
    for (const r of rs) {
      const sels = r.selector.split(',').map(s => s.trim())
      if (!sels.includes(tag)) continue
      const d = decls(r.body)
      const f = d.get('font-size')
      if (f) {
        const mm = f.match(/var\(--md-font-size\)\s*\*\s*([\d.]+)/)
        if (mm) scale = Number(mm[1])
      }
      if (d.has('margin')) {
      ...
      if (d.has('margin-top')) marginTop = d.get('margin-top')
      if (d.has('margin-bottom')) marginBottom = d.get('margin-bottom')
    }
  ...
  for (const [label, sel, prop, fallbackToken] of BLOCK_PROBES) {
    ...
    for (const r of [...rules(stripComments(fs.readFileSync(path.join(cssDir, 'skeleton.css'), 'utf8'))), ...rs]) {
      const sels = r.selector.split(',').map(s => s.trim())
      if (!sels.some(s => s === sel)) continue
      const d = decls(r.body)
      if (d.has(prop)) raw = d.get(prop)
      else if (d.has('margin')) raw = splitShorthand(d.get('margin'))[prop === 'margin-top' ? 0 : 2]
    }
...
for (const tag of ['h1', 'h2', 'h3', 'h4']) {
  ...
  for (const t of themes) {
    const r = data[t][tag]
    scales.push(r.scale)
    if (r.top !== null) tops.push(r.top)
    const ratio = r.top && r.bottom ? (r.top / r.bottom) : null
    const flags = []
    if (r.top !== null && r.top > 3.5) flags.push('间距>3.5em')
    else if (r.top !== null && r.top > 2.6) flags.push('间距偏大')
    if (r.scale > (CORPUS[tag].scale + 0.25)) flags.push('字号超语料P90')
    if (ratio !== null && (ratio < 1.6 || ratio > 2.6)) flags.push(`比${ratio.toFixed(2)}`)
    console.log(
      t.padEnd(14)
...
for (const t of themes) {
  for (const tag of ['h1', 'h2', 'h3', 'h4']) {
    const r = data[t][tag]
    if (r.top !== null && r.top > 3.5) over.push(`${t}.${tag}=${r.top.toFixed(2)}`)
    if (r.scale > 1.75) over.push(`${t}.${tag} 字号 ${r.scale}`)
  }
}
